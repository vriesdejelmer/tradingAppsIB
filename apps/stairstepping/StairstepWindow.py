...
class StairstepWindow(QMainWindow, Stairstep_UI):
...
#        self.setupAlignment()
    ...
    def setData(self, stock_ids, low_counts, low_moves, high_counts, high_moves):

        self.bull_table.clearContents()
        self.bull_table.setRowCount(low_counts.shape[0])
        self.bear_table.clearContents()
        self.bear_table.setRowCount(high_counts.shape[0])
        ...
        for index, stock_id in enumerate(stock_ids):
            ...
            self.bear_table.setItem(index, 0, QTableWidgetItem(self.stock_list[stock_id]['symbol']))


        for row_index in range(low_counts.shape[0]):
            for column_index in range(low_counts.shape[1]):
                print(str(low_moves[row_index, column_index]))
                intensity = 5 * (low_counts[row_index, column_index] + low_moves[row_index, column_index])
                item = QTableWidgetItem(f"{low_counts[row_index, column_index]}  * {low_moves[row_index, column_index]:.2f}%")
                item.setBackground(QBrush(QColor(255, 255, max(255-intensity,0))))
                self.bull_table.setItem(row_index, column_index+1, item)

        for row_index in range(high_counts.shape[0]):
            for column_index in range(high_counts.shape[1]):
                intensity = 5 * (high_counts[row_index, column_index] - high_moves[row_index, column_index])
                item = QTableWidgetItem(f"{high_counts[row_index, column_index]}  * {high_moves[row_index, column_index]:.2f}%")
                item.setBackground(QBrush(QColor(255, 255, max(255-intensity,0))))
                self.bear_table.setItem(row_index, column_index+1, item)
